...
//  console.log(report_bucket_final)
  ...
  async function edit_student_report (Date,principal_id,dean_id,action_taker_id,change)
  {
  
       let fulldata = await User.find({main_date:Date,action_taker_id:action_taker_id})   
        ...
        a2.data.map((c1) =>
      {
            ...
            if(c1.principal_id == principal_id && c1.dean_id == dean_id && c1.action_taker_id == action_taker_id)
            {
    ...
    {
    await User.updateMany({main_date:Date,action_taker_id:action_taker_id},{$set: {data:final_edit}})
    return {"success":true}}
    else
    {
...
async function create_special (date2,day2,action_taker_id) {

  let attendace_object =[]
  let dummy4=[]
   dummy4 = await User.find({"main_date":date2,"action_taker_id":action_taker_id})
   let len = dummy4.length
  //  console.log(len)
  if(len == 0 )
  {
      attendace_object= await process2.get_special(date2,day2,action_taker_id)
      let json_body = { main_date: date2,data:attendace_object,action_taker_id:action_taker_id}
