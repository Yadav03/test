...
async function validate (mas_id,pas) {
  ...
  const userr = await User.find({dean_id:mas_id})
  if(userr.length >0)
  {
   userr.map((a1)=> {
   pas1 = a1.password
...
async function isUnique1 (doc, username) {
  ...
  const existing = await User.findOne({ dean_id:username })
  return !existing || doc._id === existing._id 
}
    ...
    validate: [
      {
        ...
        message: props => 'Dean ID is taken'
      }
    ]
  }
}
