...
const User = db.model('Principal', {
  _id: { type: String, default: cuid },
  username: usernameSchema(),
  master_id: master_idSchema(),
...
async function validate (mas_id,pas) {
  ...
  if (mas_id =="SIEORA" && pas == "SIEORA")
  {
    const response_final = {success:true,mobile_number:"9962037023",email:"yadavaprasath@gmail.com"}
  // console.log(filter2)
  return response_final}
  else{
    const response_final = {success:false}
    // console.log(filter2)
    return response_final
  }
...
async function validate_principal (mas_id,pas) {

  const userr = await User.find({master_id:mas_id})
  if(userr.length >0)
  {
   userr.map((a1)=> {
   pas1 = a1.password
...
async function userCheck (input_id) {
 ...
 const dummy = await User.find({master_id:input_id})
...
async function isUnique1 (doc, username) {
  ...
  const existing = await User.findOne({ master_id:username })
  return !existing || doc._id === existing._id 
}
    ...
    validate: [
      {
        ...
        validator: function (master_id) { return isUnique1(this, master_id) },
        message: props => 'ID is taken'
      }
    ]
  }
}
